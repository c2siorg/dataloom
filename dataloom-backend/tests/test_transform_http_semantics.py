...
from app.utils.pandas_helpers import read_file_safe
...
def test_transform_redacts_csv_not_found_path_detail(client, project_id, monkeypatch):
    ...
    monkeypatch.setattr(transformations_endpoint, "read_file_safe", boom)
...
def test_transform_reverts_file_if_log_transformation_fails(client, project, monkeypatch):
    ...
    original_df = read_file_safe(file_path)
    ...
    restored_df = read_file_safe(file_path)
