"""Transformation API endpoints for dataset operations.
...
from app.services.dataset_service import log_transformation
...
def _handle_basic_transform(df, transformation_input, dataset, db, dataset_id):
...
def _handle_complex_transform(df, transformation_input, dataset, db, dataset_id):
...
async def transform_dataset(
    ...
    """Apply a basic transformation to a dataset."""
    dataset = get_dataset_or_404(dataset_id, db)
    df = read_csv_safe(dataset.file_path)

    try:
        result_df, should_save = _handle_basic_transform(
            df, transformation_input, dataset, db, dataset_id
        )
    except ts.TransformationError as e:
        raise HTTPException(status_code=400, detail=str(e))

    if should_save:
        save_csv_safe(result_df, dataset.file_path)
        log_transformation(db, dataset_id, transformation_input.operation_type, transformation_input.dict())
...
async def complex_transform_dataset(
    dataset_id: int,
    transformation_input: schemas.TransformationInput,
    db: Session = Depends(database.get_db),
):
