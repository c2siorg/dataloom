import { defineConfig, devices } from "@playwright/test";

const isCI = !!process.env.CI;

export default defineConfig({
  testDir: "./e2e",
  fullyParallel: false,
  forbidOnly: isCI,
  retries: isCI ? 2 : 0,
  workers: 1,
  reporter: isCI
    ? [["github"], ["html", { open: "never" }]]
    : [["html", { open: "on-failure" }]],
  timeout: 90_000,

  expect: {
    timeout: 10_000,
  },

  // Signs up a test user once and saves the auth cookie. All tests
  // (and the per-test `request` fixture) inherit it via `use.storageState`.
  globalSetup: "./e2e/global-setup.js",

  use: {
    baseURL: "http://localhost:3200",
    storageState: "./e2e/.auth/storage-state.json",
    trace: "retain-on-failure",
    screenshot: "only-on-failure",
    video: isCI ? "retain-on-failure" : "off",
    navigationTimeout: 30_000,
    actionTimeout: 10_000,
  },

  // Chromium-only on PR CI to keep runtime/cost predictable.
  // Expand to Firefox/WebKit in a follow-up once this suite stabilizes.
  projects: [
    {
      name: "chromium",
      use: { ...devices["Desktop Chrome"] },
    },
  ],

  webServer: [
    {
      command: "cd dataloom-backend && uv run uvicorn app.main:app --port 4200",
      url: "http://localhost:4200/docs",
      reuseExistingServer: !isCI,
      timeout: 60_000, // Allow cold-start DB migration + app startup in CI
      env: {
        DATABASE_URL:
          process.env.DATABASE_URL ??
          "postgresql://localhost:5432/dataloom",
        // Auth settings: dev-only fallbacks so local runs work without a .env.
        // CI overrides these via the workflow's .env step.
        JWT_SECRET:
          process.env.JWT_SECRET ??
          "e2e-local-jwt-secret-not-for-production-use-only",
        COOKIE_SECURE: process.env.COOKIE_SECURE ?? "false",
        SMTP_USERNAME: process.env.SMTP_USERNAME ?? "",
        SMTP_PASSWORD: process.env.SMTP_PASSWORD ?? "",
        SMTP_FROM_EMAIL: process.env.SMTP_FROM_EMAIL ?? "",
      },
    },
    {
      command: "cd dataloom-frontend && npm run dev",
      url: "http://localhost:3200",
      reuseExistingServer: !isCI,
      timeout: 60_000, // 60s — generous for Vite in CI
    },
  ],
});
