...
import QualityAssessmentView from "../quality/QualityAssessmentView";
 ...
 * assessment, sharing state via QualityViewContext.
 *
 ...
 * runs stay explicit: after a transform the assessment is marked stale and offered
 * a re-run instead of refreshing itself.
 */
export function QualityTab() {
  const { assessment, loading, error, stale, run, runHistory } = useQualityView();
  ...
  useEffect(() => {
    if (!autoRan.current && assessment === null && !loading && !error) {
      autoRan.current = true;
      run();
    }
  }, [assessment, loading, error, run]);
  ...
  return (
    <div className="flex-1 overflow-auto p-4">
      ...
      {error ? (
        ...
        <QualityAssessmentView assessment={assessment} history={runHistory} />
      ) : (
