...
import { getProjectDetails, updateProject } from "../../api/projects";
...
export default function SettingsProjectPage() {
  ...
  const fetchProject = useCallback(async () => {
    if (!projectId) return;
    try {
      const data = (await getProjectDetails(projectId, 1, 50)) as ProjectDetails;
      setProject(data);
    ...
    try {
      ...
      setProject((prev) =>
        prev
          ? {
              ...prev,
              ...(isName ? { filename: name.trim() } : { description: description.trim() }),
            }
          : prev,
      );
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold tracking-tight text-slate-900 mb-1">{project.filename}</h1>
      ...
      >
        <p className="text-sm text-gray-700 mb-6">
          Are you sure you want to delete <strong>{project.filename}</strong>? This will permanently
